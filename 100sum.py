...
def findSum(seq, targetSum, outword):
    if len(seq) == 0:
        if targetSum == 0:
            print("Found Sum K", outword)
        return
    for i in range(1, len(seq)):
        word = seq[0:i]
        intWord = int(word)
        print(intWord, outword)
        findSum(seq[i:],targetSum - intWord, word +"+"+outword )
        findSum(seq[i:], intWord- targetSum, word +"-"+outword )
